...
import { DiscordAPIError, HTTPError, Constants, Util } from 'discord.js';
...
function calculateReset(reset: string | number, serverDate: number) {
...
export class RequestHandler {
    ...
    async execute(request: APIRequest) {
        ...
        if (request.retries === this.manager.client.options.retryLimit) {
            throw new HTTPError(error.message, error.constructor.name, error.status, request.method);
        }
        ...
        try {
            ...
            throw new HTTPError(err.message, err.constructor.name, err.status, request.method);
        }

        throw new DiscordAPIError(request.path, data, request.method);
        }
        ...
        if (request.retries === this.manager.client.options.retryLimit) {
            throw new HTTPError(res.statusText, res.constructor.name, res.status, request.method);
        }
