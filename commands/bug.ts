import { MessageEmbed, MessageAttachment, Collection, Permissions, Message, Snowflake, TextChannel, GuildMember, MessageEmbedAuthor } from 'discord.js';
...
module.exports = {
    ...
    CHANNELS: CHANNELS,
    correctReportList: (client: Semblance, message: Message, messageID: Snowflake) => correctReportList(client, message, messageID),
    Report: Report
}

module.exports.run = async (client: Semblance, message: Message, args: string[], identifier: string, { permissionLevel, content }: any) => {
...
async function report(message: Message, content: string, client: Semblance) {
    ...
    if (message.attachments.size > 0) {
        ...
        (client.guilds.cache.get(sirhGuildID)!.channels.cache.find(c => c.name == "image-storage") as TextChannel)!.send(attachment)
    ...
    let reportURL = '';
    (message.guild!.channels.cache.get(CHANNELS.QUEUE) as TextChannel)!.send(embed).then(async (msg) => { // <-- #bug-approval-queue channel in C2S
...
async function correctReportList(client: Semblance, message: Message, messageID: Snowflake) {
