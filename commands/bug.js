...
const wait = util_1.promisify(setTimeout), { prefix, sirhGuildID, c2sGuildID } = config_1.default, cooldown = new discord_js_1.Collection(), CHANNELS = {
...
module.exports.run = async (client, message, args, identifier, { permissionLevel, content }) => {
    ...
    else if (identifier == 'report' || identifier == 'bugreport')
        return report(message, content, client);
    else if (identifier == 'bug')
        return bug(client, message, permissionLevel, content, args);
};
async function help(message, permissionLevel) {
    ...
    let embed = new discord_js_1.MessageEmbed()
        ...
        .setColor(constants_1.randomColor)
...
async function report(message, content, client) {
    ...
    if (difcontent.length < 5) {
        ...
        contentList = difcontent;
    if (contentList[0].length < 1 || contentList[1].length < 1 || contentList[2].length < 1 || contentList[3].length < 1 || contentList[4].length < 1)
        return message.reply("You missed some content in your report, please don't leave fields empty.").then(msg => setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 10000));
    let userCooldown = cooldown.get(message.author.id);
    if (userCooldown && Date.now() - userCooldown < (1000 * 60 * 5) && !message.member.permissions.has(discord_js_1.Permissions.FLAGS.MANAGE_MESSAGES))
        return message.reply(`You're on cooldown with this command, you can use the command again in ${(Date.now() - userCooldown - (1000 * 60 * 5)) / 1000 / 60} minutes.`);
    else if (!userCooldown || Date.now() - userCooldown > (1000 * 60 * 5))
        cooldown.set(message.author.id, Date.now());
    let reportHandler = await Report_1.Report.find({}), totalReports = reportHandler.map(r => r).length;
    ...
    var embed = new discord_js_1.MessageEmbed()
        ...
        .addFields({ name: "Actual Result", value: contentList[1] }, { name: "Expected Result", value: contentList[2] }, { name: "Operating System", value: contentList[3] }, { name: "Game Version", value: contentList[4] }, { name: "Can Reproduce", value: "Currently no one else has reproduced this bug." })
        .setFooter(`#${currentBugID}`)
        .setTimestamp(Date.now());
    let attachmentURL = "none";
    if (message.attachments.size > 0) {
        let attachment = new discord_js_1.MessageAttachment(message.attachments.map(a => a)[0].proxyURL, "Image.png");
        ...
        let videoType = [".mov", ".mp4", ".mkv", ".webm"], imageType = [".png", ".jpg", ".jpeg", ".gif"], foundType = false;
    ...
    let reportURL = '';
    message.guild.channels.cache.get(CHANNELS.QUEUE).send(embed).then(async (msg) => {
        let report = new Report_1.Report({ User: message.author.id, bugID: currentBugID, messageID: msg.id, channelID: msg.channel.id });
        await report.save();
        reportURL = msg.url;
    });
    message.delete();
    message.channel.send(new discord_js_1.MessageEmbed().setTitle(`Report Successfully Sent!`)
        .setURL(reportURL)
...
async function bug(client, message, permissionLevel, content, args) {
    ...
    if (!providedID)
        return message.reply("The ID you specified is invalid").then(msg => { setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 5000); message.delete(); });
    report = await Report_1.Report.findOne({ bugID: providedID });
    if (!report && !!providedID.match(/[0-9]{17,21}/))
        report = await Report_1.Report.findOne({ messageID: providedID });
    if (!report)
        return message.reply("The ID you specified doesn't exist.").then(msg => { setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 5000); message.delete(); });
    if (args[1] == 'attach')
        addAttachment(client, message, report, message.attachments.map(a => a)[0] || args.slice(2)[0]);
    else if (args[1] == 'reproduce')
        addReproduce(message, report, args);
    else if (args[1] == 'deleterepro' || args[1] == 'reprodelete')
        deleteReproduce(message, report, args);
    else if (permissionLevel >= 1) {
        var channel = message.guild.channels.cache.get(CHANNELS.APPROVED); // <-- #approved-bugs channel in C2S
        if (args[1] == 'archive')
            archiveReport(client, message, report);
        else if (args[1] == 'approve')
            fixUpReports(client, message, channel, report, args.slice(2).join(' '), true);
        else if (args[1] == 'deny')
            fixUpReports(client, message, channel, report, args.slice(2).join(' '), false);
    }
    else if (args[1] == 'approve' || args[1] == 'deny')
        message.reply("You do not have permission to use this.").then(msg => setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 5000));
    message.delete();
}
async function addAttachment(client, message, report, attachment) {
    if (constants_1.getPermissionLevel(message.member) == 0 && report.User != message.author.id)
        return message.reply("You don't have permission to add attachments to other people's reports.");
    if (!attachment)
        return message.reply("You didn't send any attachment nor a link");
...
async function deleteReproduce(message, report, args) {
    message.guild.channels.cache.get(report.channelID).messages.fetch(report.messageID, false)
        .then(msg => {
        let reproduceField = msg.embeds[0].fields[4].value.split('\n');
        let itemIndex = reproduceField.findIndex(item => item.includes(args.join(' ')));
        if (itemIndex == -1)
            return message.reply("That reproduce item doesn't exist.").then(m => setTimeout(() => { if (!m.deleted)
                m.delete(); }, 20000));
        else
            reproduceField.splice(itemIndex, 1);
...
async function addReproduce(message, report, specifications) {
    ...
    if (!sysInfo)
        return message.reply("You forgot to provide system information and game version.").then(msg => setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 5000));
    if (!gameVersion)
        return message.reply("You forgot to provide the game version.").then(msg => setTimeout(() => { if (!msg.deleted)
            msg.delete(); }, 5000));
    message.guild.channels.cache.get(report.channelID).messages.fetch(report.messageID, false)
        .then(msg => {
        ...
        if (reproduceField.value.startsWith(":white_check_mark:"))
            var editedReproduce = msg.embeds[0].spliceFields(4, 1, { name: "Can Reproduce", value: `${reproduceField.value}\n:white_check_mark:${message.author.tag}|${sysInfo}|${gameVersion}` });
        else
            var editedReproduce = msg.embeds[0].spliceFields(4, 1, { name: "Can Reproduce", value: `:white_check_mark:${message.author.tag}|${sysInfo}|${gameVersion}` });
        msg.edit(editedReproduce);
    })
        .catch(err => err);
}
async function fixUpReports(client, message, channel, report, reason = 'unspecified', approved) {
    message.guild.channels.cache.get(CHANNELS.QUEUE).messages.fetch(report.messageID, false) // <-- #bug-approval-queue channel from C2S
        .then(async (msg) => {
        ...
        if (approved) {
            ...
            await Report_1.Report.findOneAndUpdate({ messageID: report.messageID }, { $set: { messageID: m.id, channelID: m.channel.id } }, { new: true });
        }
        else {
            let m = await user.send(msg.embeds[0].setColor("#D72020").addField("Denial Message", reason)); // <-- Denied Reports
            await Report_1.Report.findOneAndDelete({ messageID: report.messageID });
        }
        msg.delete();
...
async function attachmentFieldCorrection(client, message, report, item) {
    ...
    if (/https?:\/\/(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)\w{11}/.exec(item) != null) {
        ...
        try {
            attachment = new discord_js_1.MessageAttachment(item);
...
async function correctReportList(client, message, messageID) {
    let deletedReport = await Report_1.Report.findOneAndDelete({ messageID: messageID });
    if (!deletedReport)
        return;
    let reportList = await Report_1.Report.find({});
    let bugIdList = Array.from(reportList.map(r => r.bugID).filter(item => item > deletedReport.bugID));
    bugIdList.forEach(async (bugID) => {
        let report = await Report_1.Report.findOneAndUpdate({ bugID: bugID }, { $set: { bugID: bugID - 1 } }, { new: true });
...
async function archiveReport(client, message, report) {
    ...
    try {
        ...
        await Report_1.Report.findOneAndDelete({ bugID: report.bugID });
        let reportList = await Report_1.Report.find({});
        ...
        bugIdList.forEach(async (bugID) => {
            ...
            await Report_1.Report.findOneAndUpdate({ bugID: bugID }, { $set: { bugID: bugID - 1 } }, { new: true });
        });
