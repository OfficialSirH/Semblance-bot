...
export class TopGG extends VoteHandler {
	...
	async handle(request: TGGRequest, reply: FastifyReply): Promise<FastifyReply> {
		...
		if (playerData) playerData = await Game.findOneAndUpdate({ player: vote.user }, 
		{ $set: { money: playerData.money + (playerData.idleProfit * earningsBonus) } }, { new: true });
			
		this.sendVotedEmbed(user ?? vote.user, `Thanks for voting for Semblance on ${this.votingSite}!! :D`, { hasGame: !!playerData, weekendBonus: vote.isWeekend });
