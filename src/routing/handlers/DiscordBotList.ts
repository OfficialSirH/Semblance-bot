...
export class DiscordBotList extends VoteHandler {
	...
	async handle(request: DBLRequest, reply: FastifyReply): Promise<FastifyReply> {
		...
		if (playerData) playerData = await Game.findOneAndUpdate({ player: vote.id }, 
		{ $set: { money: playerData.money + (playerData.idleProfit * (3600 * 6)) } }, { new: true });
			
		this.sendVotedEmbed(user ?? vote.id, `Thanks for voting for Semblance on ${this.votingSite}!! :D`, { hasGame: !!playerData });
