...
export const middleware = (req: FastifyRequest, rep: FastifyReply) => {
    ...
    if (Authorization != process.env.BOT_LISTING_AUTH && authorization != process.env.BOT_LISTING_AUTH) return rep.status(403).send({ error: 'Invalid Authorization header' });
}