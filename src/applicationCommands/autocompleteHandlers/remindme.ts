...
export async function run(interaction: AutocompleteInteraction, options: CommandInteractionOptionResolver<AutocompleteInteraction>) {
    ...
    const { groups: { previousInputType, numInput } } = timeInputAutocompleteAssistantRegex.exec(focusedOption);
    const finalValue = previousInputType && previousInputType != 'm' ? 
    ...
    for (let i = 0; i < clamp(99 - timeValues.at(-1).value, 0, 5) + 1; i++) {
        ...
        `${finalValue.value + i}${finalValue.type}`;
