...
export default {
  ...
  run: async (interaction, { client }) => {
    if (rpsGames.has(interaction.user.id))
    ...
    let opponent: User | GuildMember = interaction.options.getUser('opponent'),
      failed = false;
    try {
    ...
    if (user.bot && user.id != client.user.id)
      return interaction.reply(
        `You can't face a bot (except for me) so what are you doing trying to fight, ${
          (opponent as GuildMember).user.tag
        }?`,
      );
    const components = [
    ...
    const awaitingText = gaveChoice
        ? `Awaiting for **${(opponent as GuildMember).user.tag}**`
        : `Awaiting for **${user.tag}** and **${(opponent as GuildMember).user.tag}**`,
