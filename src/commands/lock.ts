...
import { Collection, GuildChannel, Message, TextChannel, User } from 'discord.js';
...
module.exports.run = async (client: Semblance, message: Message, args: string[]) => {
    if (args[0] == "-public") {
        let channels: Collection<string, GuildChannel>;
...
async function lockChannel(channel: TextChannel, author: User) {
    let permission = channel.permissionOverwrites.find(po => po.id == channel.guild.roles.everyone.id);
