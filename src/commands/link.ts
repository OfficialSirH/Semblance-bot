...
const run = async (client: Semblance, message: Message, args: string[]) => {
    ...
    try {
        ...
        if (updatedUser) {
            ...
            return message.channel.send(`The link was successful, now you can use the Discord button in-game to upload your progress.`);
        }
        ...
        newUser.save(function(err, entry) {
            ...
            message.channel.send(`The link was successful, now you can use the Discord button in-game to upload your progress.`);
        });         
    }   
}