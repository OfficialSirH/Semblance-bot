...
export const filteredTweetCreate = async (client: Semblance, tweet: Tweet) => {
    const c2sTwitterChannel = client.guilds.cache.get(config.c2sGuildId).channels.cache.find(c => c.name == 'cells-tweets') as TextBasedChannels,
    sirhTwitterChannel = client.guilds.cache.get(config.sirhGuildId).channels.cache.find(c => c.name == 'cells-tweets') as TextBasedChannels,
    computerLunchTwitterChannel = client.guilds.cache.get(config.lunchGuildId).channels.cache.find(c => c.name == 'cells-tweets') as TextBasedChannels,
