...
export const run = async (interaction: MessageComponentInteraction, { action, id }: ButtonData) => {
    ...
    else if (action == 'semblance') embed.setTitle('Semblance - by cabiie')
    .setImage(interaction.client.user.displayAvatarURL()+'?size=2048');
    
    else if (action == 'semblancebeta') embed.setTitle('Semblance Beta - by Lemon')
