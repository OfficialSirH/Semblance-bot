import { RPSGame } from "@semblance/lib/interfaces/rps";
...
export const run = async (interaction: MessageComponentInteraction, { action, id }: ButtonData) => {
    ...
    if (!rpsGames.has(id) || (rpsGames.get(id).opponent.id != user.id && id != user.id)) return await interaction.deferUpdate();
    const currentGame = rpsGames.get(id);
    if ((id == user.id && !!currentGame.player.choice) || (currentGame.opponent.id == user.id && !!currentGame.opponent.choice)) return await interaction.deferUpdate();
    ...
    const updatedGame = rpsGames.get(id), { player, opponent } = updatedGame;
    ...
    if (!!player.choice && !!opponent.choice) {
        ...
        if (playerVictory == 'tie') embed.setDescription(`${player.tag} and ${opponent.tag} both chose ${player.choice}`).setTitle(`tie!`);
