...
export function reset () {
  ...
  modulePaths.forEach(function (path) {
    ...
    // if child isn't mainModule, remove it with removePathHelper
    Object.getOwnPropertyNames(require.cache).forEach(function (name) {
        var child = require.cache[name]
        if (child.parent !== mainModule) {
            removePathHelper(path, child.paths)
        }
    });
  })
...
export function init (options?: any) {
  ...
  if (npmPackage._moduleDirectories instanceof Array) {
    npmPackage._moduleDirectories.forEach(function (dir: string) {
