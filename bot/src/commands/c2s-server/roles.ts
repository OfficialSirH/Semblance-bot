...
export default class Roles extends Command {
	...
	public override async chatInputRun(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction) {
		...
		const hasServerEvents = member.roles.find((r) => r === c2sRoles.server.serverEvents);
		const components = [
	...
	public override async componentRun(
		...
		await this.client.rest.patch(Routes.channelMessage(interaction.channel_id, interaction.message.id), {
			body: {
				embeds: [embed.toJSON()],
				components
			}
