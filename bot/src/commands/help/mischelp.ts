...
export default class MiscHelp extends Command {
	...
	public override templateRun() {
		const funCommands = this.client.cache.handles.commands.filter((c) => c.category === Category.fun).map((c) => `**${c.name}**`);
		const utilityCommands = this.client.cache.handles.commands.filter((c) => c.category === Category.utility).map((c) => `**${c.name}**`);
		const semblanceCommands = this.client.cache.handles.commands.filter((c) => c.category === Category.semblance).map((c) => `**${c.name}**`);
