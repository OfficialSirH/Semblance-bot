...
export default class Lookup extends Command {
	...
	public override async chatInputRun(
		...
		// user lookup
		try {
			const user = await getUser(this.client.rest, unknownItem, this.client.cache.data.guilds.get(interaction.guild_id) as APIGuild);
			if (user) {
				if (user.bot) {
		...
		try {
			...
			if (_invite) {
				const invite = (await request(`https://discordapp.com/api/v10/invites/${_invite.code}?with_counts=true`).then((res) =>
					res.body.json()
				)) as APIInvite;

				const fields: APIEmbedField[] = [];
				const add = (values: Record<string, unknown>, inline = true) => {
					for (const name in values)
						fields.push({
							name,
							value: (typeof values[name] != 'string' ? values[name]?.toString() : <string>values[name]) || 'N/A',
		...
		// message lookup
		const channels = (this.client.cache.data.guilds.get(interaction.guild_id) as APIGuild & GatewayGuildCreateDispatchData).channels
