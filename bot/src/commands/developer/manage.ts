...
export default class Manage extends Command {
	...
	public async createGameEvent(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction, options: InteractionOptionResolver) {
	...
	public async editGameEvent(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction, options: InteractionOptionResolver) {
		const name = options.getString('name', true);
		...
		try {
			const scheduledEvent = (
				(await this.client.rest.get(Routes.guildScheduledEvents(interaction.guild_id))) as RESTGetAPIGuildScheduledEventsResult
	...
	public async componentRun(reply: FastifyReply, interaction: APIMessageComponentGuildInteraction, customData: ManageButtonCustomId) {
	...
	private async pullPlayFabData<T extends RESTPostAPIPlayFabPlayerProfileResult | RESTPostAPIPlayFabPlayerStatisticsResult | unknown = unknown>(
		route: APIPlayFabRoutes,
		playFabId: string
	): Promise<ResultValue<boolean, T>> {
...
If you're sure you did, please contact the bot owner for further assistance.`
	...
	private async createBetaTesterRoleMessage(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction) {
		const components = [
			new ActionRowBuilder<TextInputBuilder>().addComponents([
				new TextInputBuilder()
	...
	private async editBetaTesterRoleMessage(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction) {
		const components = [
