...
export default class Eval extends Command {
	...
	public override async contextMenuRun(res: FastifyReply, interaction: APIContextMenuInteraction, options: InteractionOptionResolver) {
		...
		const content = message.content.startsWith('```js\n') && message.content.endsWith('```') ? message.content.slice(6, -3) : message.content;
		...
		try {
			...
			Promise.resolve(evaled).then(async (result) => {
				...
				if (evaled.length > 1015) {
					...
					embed.addFields({ name: '📤 Output', value: 'Output is in file preview above' }).setTitle('✅ Evaluation Completed');
				} else
