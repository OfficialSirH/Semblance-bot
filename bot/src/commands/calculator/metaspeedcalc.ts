...
export default class MetaspeedCalc extends Command {
	...
	public override chatInputRun(res: FastifyReply, interaction: APIChatInputApplicationCommandGuildInteraction, options: InteractionOptionResolver) {
		...
		const dinoRanks = options.getInteger('mv_ranks') ? clamp(options.getInteger('mv_ranks') || 0, 0, 550) : 0;
		const simSpeed = options.getInteger('speed_upgrades') ? clamp(options.getInteger('speed_upgrades') || 0, 0, 2105) : 0;
