...
export function guildBookPage(client: Client, chosenPage: number) {
	...
	const numOfPages = Math.ceil(client.cache.data.guilds.size / serversPerPage);
...
export const fetchCatOrDog = async (query_params: AnimalAPIParams, wantsCat: boolean) => {
	...
	const API_KEY = wantsCat ? process.env.CAT_API_KEY : process.env.DOG_API_KEY;
...
export const SuggestionConstants = {
	...
	createUniqueKey: (userId: Snowflake, suggestion: string) => `${userId}-${suggestion.slice(-4)}` as `${Snowflake}-${string}`,
	// quarter of an hour
