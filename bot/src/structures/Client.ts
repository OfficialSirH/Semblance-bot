...
export class Client {
	...
	async loadCommands() {
		...
		for (const file of commandFiles) {
			const command = new (await import(pathToFileURL(join(__dirname, '..', 'commands', file)).toString())).default(this) as Command;
	...
	async loadPreconditions() {
		const __dirname = dirname(fileURLToPath(import.meta.url));
		const preconditionFiles = (await readdir(pathToFileURL(join(__dirname, '..', 'preconditions')))).filter((file) => file.endsWith('.js'));

		for (const file of preconditionFiles) {
			const precondition = new (await import(pathToFileURL(join(__dirname, '..', 'preconditions', file)).toString())).default(
				this
			) as Precondition;
	...
	async loadListeners() {
		const __dirname = dirname(fileURLToPath(import.meta.url));
		const listenerFiles = (await readdir(pathToFileURL(join(__dirname, '..', 'listeners')))).filter((file) => file.endsWith('.js'));

		for (const file of listenerFiles) {
			const listener = new (await import(pathToFileURL(join(__dirname, '..', 'listeners', file)).toString())).default(this) as Listener;
	...
	async deployCommands() {
		...
		const globalCommands = (
			await Promise.all(this.cache.handles.commands.filter((command) => 'data' in command).map(async (command) => command.data?.()))
		)
