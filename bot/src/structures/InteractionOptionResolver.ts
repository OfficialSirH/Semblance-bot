...
export class InteractionOptionResolver {
	...
	public constructor(interaction: APIChatInputApplicationCommandInteraction | APIApplicationCommandAutocompleteInteraction) {
		this.resolved = (interaction.data.resolved as APIInteractionDataResolved & { messages: Record<Snowflake, APIMessage> }) ?? undefined;
		...
		if (interaction.data.options?.[0].type === ApplicationCommandOptionType.SubcommandGroup) {
			...
			this.options = interaction.data.options[0].options?.[0].options?.map((option) => new InteractionOption(option)) ?? [];
	...
	public getMentionable(name: string, throwIfNotHere = false): APIUser | APIRole | APIInteractionDataResolvedGuildMember | undefined {
		const option = this.get(name);
		if (!option) {
			if (throwIfNotHere) throw new Error(`Option "${name}" not found`);
			return undefined;
		}
		if (option.type !== ApplicationCommandOptionType.Mentionable) throw new Error(`Option "${name}" is not a mentionable`);
		return (
	...
	public getAttachment(name: string, throwIfNotHere = false): APIAttachment | undefined {
		...
		if (option.type !== ApplicationCommandOptionType.Attachment) throw new Error(`Option "${name}" is not an attachment`);
