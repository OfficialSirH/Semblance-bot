...
From ${shortFormattedDate(start)} until ${shortFormattedDate(end)}, get another chance to collect all three fungi badges! 🏆✨`,
...
Play from ${shortFormattedDate(start)} until ${shortFormattedDate(
	...
	'Human Body': {
      ...
      Connect like never before in Explore: Human Body - Anatomy of Life from ${shortFormattedDate(start)} to ${shortFormattedDate(end)}`,
