...
export const resolveBodyWithAttachments = async (resolveableBody: RESTPostAPIChannelMessageJSONBody & { files?: Attachy[] }) => {
...
export const getChannel = (search: string | Snowflake, guild: APIGuild & GatewayGuildCreateDispatchData) => {
	...
	return channels.find((ch) => search.toLowerCase() == ch.name?.toLowerCase()) ?? channels.find((ch) => ch.id === getId(search));
};
