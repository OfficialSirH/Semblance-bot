...
export const filterAction = (
	...
	components.map((row) =>
		row.setComponents(
			row.components.filter((c) => JSON.parse((c.data as APIButtonComponentWithCustomId | APISelectMenuComponent).custom_id).action != action)
		)
	);
 ...
 */
export interface ComponentInteractionDefaultParserOptions<T extends CustomIdData = CustomIdData> {
	allowOthers?: boolean;
...
export const buildCustomId = <T extends CustomIdData | (CustomIdData & Record<string, unknown>) = CustomIdData>(customIdOptions: T) =>
