	...
	const job = new CronJob(
		...
		async () => {
			...
			try {
				...
				console.log(`[EventScheduler] List of found events from server: ${eventsFromGlobalVars.map((e) => e.name).join(', ')}`);
			} catch (error) {
			...
			try {
				const existingEventsResponse = await client.core.rest.get(Routes.guildScheduledEvents(GuildId.cellToSingularity));

				const existingEvents = existingEventsResponse as APIGuildScheduledEvent[];

				if (existingEvents.length > 0) {
					existingEvents.forEach((event) => {
						console.log(`Event: ${event.name} | Start: ${event.scheduled_start_time} | End: ${event.scheduled_end_time}`);
					});
				} else {
