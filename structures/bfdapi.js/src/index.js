...
class BFDAPI extends EventEmitter {
  /**
   * Creates a new BFDAPI Instance.
   * @param {string} token Your botsfordiscord.com token for this bot.
   * @param {Object} [options] Your BFDAPI options.
   ...
   * @param {string} [options.webhookPath='/bfdwebhook'] The path for the webhook request.
  ...
  constructor(token, options, client) {
    ...
    if (client && isASupportedLibrary(client)) {
      ...
      console.error(`[bfdapi.js autopost] The provided client is not supported.`); // eslint-disable-line no-console
    }

    if (this.options.webhookPort || this.options.webhookServer) {
      const BFDWebhook = require('./webhook');
      this.webhook = new BFDWebhook(this.options.webhookPort, this.options.webhookPath, this.options.webhookAuth, this.options.webhookServer);
    }
  }
  ...
  _request(method, endpoint, data) {
    return new Promise((resolve, reject) => {
      ...
      if (this.token) {
        ...
        console.warn('[bfdapi.js] Warning: No DBL token has been provided.'); // eslint-disable-line no-console
      }
   ...
   */
   async getWidget(id) {
    ...
    if (!id) id = this.client.user.id;
    const response = await this._request('get', `bot/${id}/widget`);
    return response.body;
  }
   ...
   */
  async getVotes() {
    const response = await this._request('get', `bot/${this.client.user.id}/votes`, undefined, true);
    return response.body;
  }
