...
class BFDWebhook extends EventEmitter {
  /**
  ...
  _handleRequest(req, res) {
    if (req.url === '/bfdwebhook' && req.method === 'POST') {
      const auth = JSON.parse(process.env.botsForDiscordAuth).webAuth;
      ...
      req.on('end', () => {
        if (data) {
           ...
           * @event BFDWehook#vote
           * @param {string} user Id of the user that voted.
           * @param {string} bot Id of the bot that was voted for.
           * @param {object} votes list of all types of votes: Number types: 'totalVotes', 'votes24', 'votesMonth'. ID Arrays: 'hasVoted', 'hasVoted24'
           * @param {object} [query] The possible querystring parameters from the vote page.
           * @param {string} type Type of the vote. Is always "vote" except when using the test button it's "test".
           */
          emit('vote', data);
