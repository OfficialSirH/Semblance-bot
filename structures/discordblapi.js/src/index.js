...
class DiscordBLAPI extends EventEmitter {
  /**
   * Creates a new DiscordBLAPI Instance.
   * @param {string} token Your discordbotlist.com token for this bot.
   * @param {Object} [options] Your DiscordBLAPI options.
   ...
   * @param {string} [options.webhookPath='/discordblwebhook'] The path for the webhook request.
  ...
  constructor(token, options, client) {
    ...
    if (client && isASupportedLibrary(client)) {
      ...
      console.error(`[discordblapi.js autopost] The provided client is not supported.`); // eslint-disable-line no-console
    }

    if (this.options.webhookPort || this.options.webhookServer) {
      const DiscordBLWebhook = require('./webhook');
      this.webhook = new DiscordBLWebhook(this.options.webhookPort, this.options.webhookPath, this.options.webhookAuth, this.options.webhookServer);
    }
  }
  ...
  _request(method, endpoint, data) {
    return new Promise((resolve, reject) => {
      ...
      if (this.token) {
        ...
        console.warn('[discordblapi.js] Warning: No DiscordBL token has been provided.'); // eslint-disable-line no-console
      }
  ...
  async postStats(serverCount, shardId, shardCount) {
    ...
    if (serverCount) {
      ...
      data.guilds = this.client.guilds.size || this.client.guilds.cache.size;
  ...
  /**
   * Gets widget from a bot.
   * @param {string} id The ID of the bot you want to get the widget from.
   * @returns {Promise<Object>}
   */
   async getWidget(id) {
    if (!id && !this.client) throw new Error('getWidget requires id as argument');
    if (!id) id = this.client.user.id;
    const response = await this._request('get', `bots/${id}/widget`);
    return response.body;
  }
   ...
   */
  async getVotes() {
    const response = await this._request('get', `bots/${this.client.user.id}/votes`, undefined, true);
    return response.body;
  }
